...
int do_login(int acceptfd, MSG *msg, sqlite3 *db)
{
	...
	if(sqlite3_get_table(db, sql, &resultp, &nrow, &nclounm, &errmsg) != SQLITE_OK)
	{
		printf("%s\n", errmsg);
		return -1;
	}
...
int do_searchword(int acceptfd, MSG *msg, char word[])
{
	...
	while (fgets(temp, 512,fp) != NULL)
	{
		...
		//表示找到了，查询的单词
		p = temp + len; // abandon  v. asgs safasf
		while (*p == ' ')
		{
			p++;
		}
...
int do_query(int acceptfd, MSG *msg, sqlite3 *db)
{
	...
	//表示找到了单词，那么此时应将应户名，时间，单词，插入到历史记录中去。
	if(found == 1)
	{
		//需要获取系统时间	
		get_date(date);	
